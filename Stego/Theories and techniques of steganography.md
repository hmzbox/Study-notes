...
- [第3章 隐写分布特性的保持](#第3章-隐写分布特性的保持)
  ...
  - [3.4基于调整修改方式的隐写(F3/F4/F5)](#34基于调整修改方式的隐写f3f4f5)
    - [F3隐写（改进值对保持对称、修改=绝对值减1、重传更多0、制造了更多的偶数）](#f3隐写改进值对保持对称修改绝对值减1重传更多0制造了更多的偶数)
    - [F4隐写（改进F3、负偶数/正奇数代表1，负奇数/正偶数代表0、修改=绝对值减1、解决了偶数变多问题，但仍存在向0收缩）](#f4隐写改进f3负偶数正奇数代表1负奇数正偶数代表0修改绝对值减1解决了偶数变多问题但仍存在向0收缩)
    - [F5隐写（矩阵编码）](#f5隐写矩阵编码)
    - [自适应思想的MMEx](#自适应思想的mmex)
  - [第五章专用隐写分析](#第五章专用隐写分析)
    - [专用隐写分析VS通用隐写分析](#专用隐写分析vs通用隐写分析)
    - [RS分析（针对LSBR）](#rs分析针对lsbr)
...
#### F3隐写（改进值对保持对称、修改=绝对值减1、重传更多0、制造了更多的偶数）
...
#### F4隐写（改进F3、负偶数/正奇数代表1，负奇数/正偶数代表0、修改=绝对值减1、解决了偶数变多问题，但仍存在向0收缩）
论文（2-F5-A steganographic algorithm High capacity despite better steganalysis(2001,IH,CR1118)
