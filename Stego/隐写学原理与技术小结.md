...
### 总结：YUV色彩模型来源于RGB模型，YCbCr模型来源于YUV模型，还未涉及压缩，压缩是JPEG通过YUV411或YUV422**采样**实现
...
3. DCT变换。将输入分块均-128,范围 [0,255] 到 [-128,127] 以满足DCT输入条件（偶函数）；每个8×8分块都通过DCT变换得到8×8的DCT系数（最左上角的点为直流系数DC，其余为交流系数AC）。
4. 量化。
5. 无损压缩。
